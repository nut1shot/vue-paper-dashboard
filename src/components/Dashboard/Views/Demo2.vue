<template>
	<div>
		<div class="card cam_btn" id='cam_btn' style='height:70px'>
            ...
            <button @click='to(5)'>[5]</button><BR/><BR/>
            <font v-if='step==1' color="white">STEP1 : ID CARD</font> 
            <font v-if='step==2' color="white">STEP2 : LASER CODE</font> 
            <font v-if='step==3' color="white">STEP2 : SELFIE</font> 
            <font v-if='step==4' color="white">STEP3 : Record VDO</font> 
            <font v-if='step==5' color="white">STEP4 : FILL-IN FORM</font> 
            </center>
		</div>
       ...
       <div v-if='isLoading'>
        ...
        <!-- Form in step 4 -->
		<div class="cam" id='form' style='height:200px' v-if="step===5 && !isLoading">
          ...
          <tr>
                  <td>PROVINCE</td>
                  <td><input type='' v-model='province'></td>
			    </tr>
               <tr>
		...
		<div class="cam" id='cam' style='height:200px' v-show='step<5 && !isLoading && !isConfirming'>
			<center>
            <select id="video_dev" v-show="step<4" onchange="chvdo3(this[this.selectedIndex].value)"
               style="font-size:16pt">
				<option>choose camera</option>
            </select>

            <!-- reader -->
            <span v-if="step===4">Read Texts Below clearly:<br/>
                  <span>
    ...
    methods: {
      to: function (n) {
        ...
        if (this.step < 5) {
          ...
          if (this.step === 2 && this.backCam) {
            activeCam = this.backCam
          }
          if (this.step === 3 && this.frontCam) {
      ...
      setActiveCam: function (camId) {
        ...
        if (camId) {
          window.chvdo3(camId, this.step === 4)
...
window.chvdo3 = function (deviceId, audio) {
    if (window.stream) {
...
function initVideo () {
    ...
    if (!window.stream) {
      ...
      var vdoDev = document.getElementById('video_dev')
      window.chvdo3(vdoDev[vdoDev.selectedIndex].value)
    } else {
